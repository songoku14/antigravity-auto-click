...
> [!IMPORTANT]
> Hãy luôn khởi động IDE bằng lệnh `antigravity`. Nếu quên, bạn có thể dùng tính năng **Restart Antigravity (Debug Mode)** trong Control Center của Extension.
...
| **Daemon không khởi động được** | Kiểm tra xem có tiến trình `node src/core/auto-retry.js` nào đang treo không bằng lệnh `pgrep -f auto-retry`. Dùng **Stop All** trong CLI để dọn dẹp. |
