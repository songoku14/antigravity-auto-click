...
async function analyzeDialog() {
  ...
  try {
    ...
    for (const target of pageTargets) {
      ...
      if (!analysis.found) {
        console.log('   [\x1b[33m⚠️\x1b[0m] Không thấy dialog chuẩn, đang quét toàn bộ trang...');
      } else {
        anyDialogFound = true;
      ...
      for (let i = 0; i < analysis.containers.length; i++) {
        ...
        if (btnText) {
          ...
          await saveSample(target.title, c, btnText, category);
        }
      }
    }
...
function runAnalysisCommand(target) {
  return new Promise((resolve) => {
    ...
    ws.on('open', () => {
      // Combined call to ensure function is available immediately
      const injectionCode = getInjectionScript();
      const combinedCode = injectionCode + "\n(typeof window.__analyzeDialog === 'function' ? window.__analyzeDialog() : 'failed_to_init');";

      ws.send(JSON.stringify({
        id: 1,
        method: 'Runtime.evaluate',
        params: {
          expression: combinedCode,
          returnByValue: true
        }
      }));
    });
      ...
      try {
        ...
        if (msg.id === 1) {
          if (msg.exceptionDetails) {
            console.error(`   [!] Analysis failed on ${target.title}:`, msg.exceptionDetails.exception.description);
            finished = true;
            ws.terminate();
            resolve(null);
            return;
          ...
          if (result === 'failed_to_init' || result === 'not_injected') {
            resolve(null);
          } else {
            resolve(result);
          }
    ...
    setTimeout(() => {
      if (!finished) {
        console.warn(`   [!] Timeout waiting for analysis on ${target.title}`);
        finished = true;
        ws.terminate();
        resolve(null);
      }
...
async function getNextSampleId(samplesDir) {
  ...
  let maxId = 0;
  files.forEach(file => {
    const match = file.match(/^sample_(\d+)\.json$/);
    if (match) {
...
async function saveSample(targetTitle, container, expectedBtn, category) {
  ...
  const nextId = await getNextSampleId(samplesDir);
  const idStr = String(nextId).padStart(3, '0');
  const filename = `sample_${idStr}.json`;
  ...
  const htmlPath = path.join(samplesDir, `sample_${idStr}.html`);
  fs.writeFileSync(htmlPath, `<!-- ID: ${idStr} -->\n<!-- Category: ${category} -->\n<!-- Target: ${targetTitle} -->\n<!-- Expected Button: ${expectedBtn} -->\n` + container.html);

  console.log(`\n   💾 Đã lưu dữ liệu mẫu [#${idStr}]:`);
  console.log(`      - JSON: samples/${filename}`);
  console.log(`      - HTML: samples/sample_${idStr}.html`);
}
