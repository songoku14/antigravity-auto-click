...
3. **Check Logs**: Tail last 10 lines of `~/Library/Logs/AntigravityAutoRetry/stdout.log`.
4. **End-to-End Test (Optional)**: If verification is needed, run `node scripts/trigger-test.js` to ensure the script reacts to UI changes.
5. **Report**: State (Running/Stopped), any errors, and test result if executed.
