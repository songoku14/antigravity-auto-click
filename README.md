...
- **Thống kê lý do bỏ qua (Skip Reasons)** (CLI > Option 2 > Option 5): Hiển thị bảng tổng hợp các lý do hệ thống đã bỏ qua không click (Rate limit, Visibility, UI Filters...).
- **Thống kê Accept theo category** (CLI > Option 2 > Option 6): Tổng hợp số lần Auto-Accept click thực tế theo từng nhóm `terminal` / `review` / `system`.
- **Load lại dữ liệu** (CLI > Option 2 > Option 7).
- **Reset bộ đếm thống kê** (CLI > Option 2 > Option 8).
